...
module.exports.get_resolution = function(metadata, maxRes, snapping, callback) {
    ...
    var spatialRes = raster.getValidSpatialResolutions(validSpatialResolutions, cellSize[0]);

    var snapRes, snapSpatialRes;

    if (snapping) {
        snapRes = spatialRes[spatialRes.length - 1] - EPSLN;
        snapSpatialRes = raster.getValidSpatialResolutions(validSpatialResolutions, snapRes);
...
module.exports.metatile_size = function(resolution, breakZooms, callback) {
    if (breakZooms.length === 0) return callback(new Error('must have at least one zoom break'))
    // sort by zoom
    breakZooms = breakZooms.sort(function(a, b) {
        return a.z < b.z;
    });

    // check if res steps match
    breakZooms.sort(function(a, b) {
        if (a.res >= b.res) return callback(new Error('resolution steps must match zooms'));
    });

    return callback(null, breakZooms.filter(function(r) {
        if (!r.res || resolution <= r.res) {
            return r.z
        }
    }));
}